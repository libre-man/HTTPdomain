...
from pygments.token import Name, Text, Number, Keyword, Literal, Operator
...
from sphinx.domains.python import PyField, PythonDomain, PyTypedField, PyGroupedField
...
class HTTPResource(ObjectDescription):
    ...
    doc_field_types = [
        ...
        Field('singlejsonvalue', label='Response JSON Object', has_arg=False,
              names=('returns', 'return')),
        PyField('singlejsontype', label='Response JSON type', has_arg=False,
                names=('rtype',), bodyrolename='obj'),
        PyTypedField('responsejsonarray', label='Response JSON Array of Objects',
        ...
        PyGroupedField('exceptions', label='Possible error responses',
                       rolename='exc', names=('raises', 'raise', 'exception',
...
class HTTPDomain(Domain):
    ...
    def __init__(self, *args, **kwargs):
        self._python = PythonDomain(*args, **kwargs)
        super(HTTPDomain, self).__init__(*args, **kwargs)
    ...
    def resolve_xref(self, env, fromdocname, builder, typ, target,
        ...
        try:
            ...
            if role is None:
                resu = self._python.resolve_xref(env, fromdocname,
                                                 builder, typ,
                                                 target, node,
                                                 contnode)
                return resu
